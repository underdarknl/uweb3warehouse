...
class Product(model.Record):
  ...
  def FromName(cls, connection, name, conditions=None):
    ...
    with connection as cursor:
      product = cursor.Select(table=cls.TableName(),
                              conditions=['name=%s' % safe_name, NOTDELETED] +
                              conditions)
    if not product:
    ...
    for part in parts:
      ...
      if part['amount']:
        part['availableassemblies'] = int(
            (part['availablestock'] +
             part['availablepossiblestock']['available']) / part['amount'])
        if part['availableassemblies'] < availableassemblies:
          limitedby = part
        availableassemblies = min(availableassemblies,
                                  part['availableassemblies'])
  ...
  def Disassemble(self, amount=1, reference="Disassembled for parts", lot=None):
    ...
    return self.Assemble(amount * -1, reference or 'Disassembled for parts',
                         lot)
  ...
  def AssemblyOptions(self):
    ...
    return self.List(self.connection,
                     conditions=[
                         'ID != %d' % self.key,
                         'ID not in (%s)' %
                         ','.join(partIds) if partIds else 'true'
                     ])
